...
from token_classification.data_utils import (NEXT_GUARD, NEXT_TOKEN,
                                        TransformerNerDataProcessor,
                                        batch_to_model_inputs,
                                        convert_features_to_tensors,
                                        ner_data_loader,
                                        transformer_convert_data_to_features)
from token_classification.model import (XLMRobertaNerModel)
from token_classification.model_utils import PGD, FGM, get_linear_schedule_with_warmup
...
def run_task(args):
    ...
    # training
    if args.do_train:
        if args.model_type in {"xlm-roberta"}:
            # we need to set add_prefix_space to True for roberta, longformer, and Bart (any tokenizer from BPE)
            tokenizer = model_tokenizer.from_pretrained(args.tokenizer_name, do_lower_case=args.do_lower_case, add_prefix_space=True)
        else:
